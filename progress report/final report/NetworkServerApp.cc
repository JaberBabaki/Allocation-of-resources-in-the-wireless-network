        ...
        if(knownNodes[i].srcAddr == pkt->getTransmitterAddress())
        {
            ...
            if(knownNodes[i].framesFromLastADRCommand == 20)
            {
                ...
                if(adrMethod == "avg")
                {
                    ...
                    {
                        totalSNR+=*it;
                        //recordScalar("totalSNR",*it);
                        numberOfFields++;
                    }
                    recordScalar("totalSNR",0000);
                    SNRm = totalSNR/numberOfFields;
                    recordScalar("totalSNR",SNRm);
                }
                if(adrMethod == "best")
                {
                    ...
                    double pathlossRate=(end-begin-18)/(end-begin);
                    double successful=pathlossRate;
                    successful = (int)(successful * 100000 + .5);
                    successful=successful / 100000;
                    knownNodes[i].adrListSNIR.sort();
                    double totalSNR = 0;
                    double we=0;
                    int ite = 19;
                    for (std::list<double>::iterator is=knownNodes[i].adrListSNIR.begin(); is != knownNodes[i].adrListSNIR.end(); ++is)
                    {
                        //recordScalar("totalSNR",*is);
                        if(ite==1){
                            we=pow(successful,(19-ite));
                        }else{
                            we=(1-successful)*pow(successful,(19-ite));
                        }
                        ...
                        /*if(ite==1){
                            ...
                            we=pow((1-successful),(ite-1));
                        }else{
                            we=successful*pow((1-successful),(ite-1));
                        }*/
                        we = (int)(we * 100000 + .5);
                        we=we / 100000;

                        totalSNR=(*is * we)+totalSNR;
                        //recordScalar("totalSNR",we);
                        ite=ite-1;
                    }
                    //recordScalar("totalSNR",begin);
                    //recordScalar("totalSNR",end);

                    //recordScalar("totalSNR",successful);
                    //recordScalar("totalSNR",totalSNR);
                    //recordScalar("totalSNR",size);
                    //recordScalar("totalSNR",100000);
                    //totalSNR = 1;
                    SNRm = totalSNR; //totalSNR/numberOfFields;
                }
